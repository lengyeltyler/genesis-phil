'use strict';
const { AbiCoder, Interface, keccak256, getCreate2Address, concat, toBeHex, zeroPadValue, getBytes, hexlify } = require('ethers');
...
function deriveAccount(config, identity, mode) {
  ...
  const runtime = getBytes(config.accountRuntimeTemplate);
  for (const [name, references] of Object.entries(config.accountImmutables)) {
    ...
    for (const {start,length} of references) {
      ...
      runtime.set(getBytes(zeroPadValue(values[name], 32)), start);
    }
  }
  ...
  return { profile, runtime: hexlify(runtime),
