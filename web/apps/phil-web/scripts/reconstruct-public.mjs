...
const reference = JSON.parse(await readFile(resolve(root, "reference-release.json")));
const originalMap = JSON.parse(await readFile(resolve(root, "ORIGINAL-SOURCE-MAP.json")));
for (const row of originalMap.files) {
  if (sha(await readFile(resolve(root, row.path))) !== row.sha256) throw Error("Public source mismatch: " + row.path);
}
const commit = reference.commit, sources = reference.sources, sourceDigest = reference.sourceDigest;
if (preview) throw Error("This public reconstruction targets the frozen production release only.");
...
if (sha(bytes) !== sha(await readFile(resolve(root, "reference-release.json")))) throw Error("Reconstructed release differs from the frozen release.");
console.log("PASS — exact Web assets, Worker and release manifest reconstructed.");
