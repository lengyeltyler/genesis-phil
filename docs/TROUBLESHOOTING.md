...
| “Mainnet advanced during verification” | You are using an older build. Install Build 87 from the official download. It tolerates routine block advances and keeps your existing Build 85/86 identity and selection. |
| Phil locked or logged out | Unlock with your Phil passphrase. Build 87 has no fixed 15-minute browsing timeout. Manual lock, Mac sleep/lock and quitting still end the session. Check your kept selection before continuing. |
...
| An older Phil identity is already installed | Keep the old identity and verified backup. Builds 85–87 share their configuration; older collection bindings require a separately planned migration. Do not delete Application Support or Keychain entries. |
