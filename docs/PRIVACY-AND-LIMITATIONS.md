...
Build 96 preserves the Build 92 deployment and account binding. Build 91 and earlier collection bindings do not transfer. There is no fixed 15-minute browsing timeout, but Mac lock/sleep, manual lock and quitting still lock Phil. Fresh confirmation and short-lived, single-use signing grants remain required.

The owner confirmed earlier Mac mini mint, transfer and withdrawal flows. Build 96 changes the accent stylesheet and build number only; no new Mainnet transaction was performed for its UI audit. The separate MacBook Pro test remains pending. macOS 12 is the build target, not evidence of physical testing on every older OS. The current release supports Apple Silicon Macs only.
